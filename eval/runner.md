...
4. Decision rule (pre-registered): Wilcoxon p < 0.05 on q-totals, Cohen's d 95% bootstrap CI lower bound > 0.2, no significant adversarial regression (McNemar one-sided p < 0.05; raw count uses pass_fraction drop > 0.5, making noise-only 2/3→1/3 flips at k=3 non-counting), zero canary leaks.
