#!/usr/bin/env bash
# PreToolUse hook: requires an @critic verdict before any `git commit` or `glab mr create`.
# Blocks (exit 2) the mutation and emits an instruction for the lead agent to invoke @critic
# on the planned diff / MR draft. The agent re-runs the command after creating a single-use
# marker file (`~/.claude-critic-approved`, TTL 5 minutes) signalling that critic returned
# `approve`. The marker is consumed on use — every subsequent commit / MR-create needs a
# fresh critic pass.
#
# Rationale: the @critic sub-agent already exists in this plugin's routing table but is
# invoked only when the lead agent remembers to. Two cheapest moments to land neuroslop in
# shared state are the commit and the MR-create. Making the critic call mandatory there
# is the smallest enforceable defence layer.
#
...
1. Invoke the critic sub-agent with:
   - for `git commit` — the output of `git diff --staged` plus the proposed commit message;
