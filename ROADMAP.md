...
| N3 | role-invariants index off-by-one | `scripts/role-invariants.sh` — `sed` read file lines, but line 1 of `invariants.txt` is the header comment, so invariant #N sat on file line N+1 | Sub-agents inherited the wrong invariant lines under `<inherited-invariants>`. | shipped (S3) |
...
| N6 | Hard-prohibitions have no deterministic guard | `hooks/hooks.json`, `scripts/` — no `PreToolUse` hook blocks `git push` to main / `--force` / `--no-verify`; `invariants.txt #24` is advisory + sub-agent propagation only | An irreversible shared-history action is guarded only by a ~1-in-25 sampled self-check and prose, not a deterministic gate. Add a `PreToolUse` red-line guard as the durable backstop. | open |
