...
| N10 | No skill-consult check before agent spawn | `hooks/` — no `PreToolUse` reminder fires a skill-match check before an `Agent` spawn whose description matches a skill trigger | Mechanical backstop for N9: text guidance erodes under load (same class as N6); a deterministic pre-spawn nudge would catch the miss N9 documents. | open |
...
| S6 | Deterministic redline guard (N6) — PreToolUse hook on Bash that mechanically blocks force-push, --no-verify, local merge/rebase onto main, PR/MR merge/close/delete, and production k8s/helm mutations; exits 2 on positive match, 0 otherwise; wired as the first Bash hook (before verification-gate and auto-critic) | `scripts/redline-guard.sh`, `scripts/selftest_redline-guard.sh`, `hooks/hooks.json` | branch `harness/door-2-mainpush-delegation` |
